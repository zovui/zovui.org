<template>
    <ComponentExampleContainer
        component-name="Slider 布局"
        desc="滑动输入器，用于在数值区间/自定义区间内进行选择，支持连续或离散值。"
        :api-list="apiList"
    >
        <ComponentExample
...
const SliderApi = [
        ...
        data: [
            {
                ...
                default: '0',
                values: '-',
                explain: 'Slider组件的值，可用v-model 双向绑定数据'
            },
            {
                prop: 'min',
                type: 'number',
            ...
            },
            {
                ...
                required: 'false',
                default: 'default',
                values: ['default', 'hidden', 'always'].join(' | '),
                explain:
                    '控制点tooltip展示规则，default只有在拖拽时展示；hidden总是隐藏；always为总是展示'
            },
            {
                prop: 'tipFormatter',
                type: 'function',
            ...
            },
            {
                ...
                default: '-',
                values: '-',
                explain: '仅在设置marks时生效，是否只能拖拽到marks上'
            },
            {
                prop: 'vertical',
                type: 'boolean',
    ...
    },
    {
        ...
        data: [
            {
                event: 'on-change',
                callbackParam: 'Slider最后的值',
                explain: '当Slider值发生改变时触发的事件'
            }
        ]
    }
]
...
export default {
    ...
    data() {
        return {
            examples: [Base, MinMax, TooltipVisible, TipFormatter, Range],
            apiList: [...SliderApi]
        }
    }
}
